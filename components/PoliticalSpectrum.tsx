...
const SpectrumLoader: React.FC = () => (
    <div className="space-y-3 animate-pulse">
        ...
        <div className="h-3 bg-slate-300 dark:bg-slate-600 rounded-full w-full"></div>
...
export const PoliticalSpectrum: React.FC<PoliticalSpectrumProps> = ({ score, isLoading }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-lg border border-slate-200 dark:border-slate-700 transition-colors duration-200">
      ...
      {isLoading ? <SpectrumLoader /> : (
        <div>
          <div className="relative h-3 w-full bg-gradient-to-r from-blue-500 via-purple-500 to-red-500 rounded-full">
            <div
              className="absolute top-1/2 -translate-y-1/2 w-5 h-5 bg-slate-700 dark:bg-slate-200 rounded-full border-2 border-white dark:border-slate-800 shadow-lg transition-all duration-500 ease-in-out"
              style={{ 
