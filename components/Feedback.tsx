...
const ThumbsUpIcon: React.FC<{className?: string}> = ({className}) => (
    <svg xmlns="http://www.w.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={className} aria-hidden="true">
        ...
        <div className="flex items-center justify-center">
            {hasGivenFeedback ? (
                ...
                <div className="flex items-center gap-6">
                    <p className="text-sm font-medium text-slate-400">Was this summary helpful?</p>
                    <div className="flex items-center gap-2">
                        <button 
                            onClick={() => onFeedback('good')} 
                            disabled={hasGivenFeedback}
                            className={`p-2 rounded-full transition-colors duration-200 ${feedback === 'good' ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-400 hover:bg-slate-700/50 hover:text-emerald-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-emerald-500'}`}
                        ...
                        <button 
                            ...
                            disabled={hasGivenFeedback}
                            className={`p-2 rounded-full transition-colors duration-200 ${feedback === 'bad' ? 'bg-red-500/20 text-red-400' : 'text-slate-400 hover:bg-slate-700/50 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-red-500'}`}
